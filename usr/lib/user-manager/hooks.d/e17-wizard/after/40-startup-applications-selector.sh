...
main(){
    ...
    do
        ...
        if [[ -n "$EROOT" ]] ; then
            ...
            if echo "$filename" | LC_ALL=C grep -qsEi "^(kde|glipper-|print-applet|notification-daemon|user-dirs-update-gtk|elive-support-donations|elive-listen-commands|org.gnome.SettingsDaemon|gnome-software-service|tracker-store|pulseaudio|at-spi)" ; then
            ...
            # E17 / E22
            if echo "$filename" | LC_ALL=C grep -qsEi "^(kde|glipper-|nm-applet|wicd-|print-applet|notification-daemon|user-dirs-update-gtk|elive-support-donations|elive-listen-commands|org.gnome.SettingsDaemon|gnome-software-service|tracker-store|pulseaudio|at-spi)" ; then
                # glipper: we want to enable it in a different way: if ctrl+alt+c si pressed, run it for 8 hours long and close/kill it to save mem
                # nm-applet: already integrated in elive correctly and saving mem
                # wicd-: deprecated and not needed for elive
                # print-applet: useless
