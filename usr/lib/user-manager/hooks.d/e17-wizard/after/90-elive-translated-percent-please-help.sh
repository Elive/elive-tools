    ...
    if [[ -n "$LANG" ]] && [[ "$LANG" != en* ]] ; then
        if [[ -d "$dir" ]] ; then
            ...
            if echo "$percent" | grep -qs "[[:digit:]]" ; then
                ...
                message_translated="Elive is translated in a ${percent}% to your language, you can improve these translations by using the application called eltrans and it will help everybody to enjoy them."
