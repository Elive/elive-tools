...
main(){
    ...
    if [[ -f "$HOME/.xchat2/xchat.conf" ]] ; then
        ...
        if [[ -n "$ip" ]] ; then
            ...
            sed -i "s|irc_nick1 = Elive_user|irc_nick1 = Elive_user_${LANG%%_*}-${NUMBERRANDOM}|" "${HOME}/.xchat2/xchat.conf"

            NUMBERRANDOM="${md5ip:2:2}"
            sed -i "s|irc_nick2 = Elive_user|irc_nick2 = Elive_user_${LANG%%_*}-${NUMBERRANDOM}|" "${HOME}/.xchat2/xchat.conf"

            NUMBERRANDOM="${md5ip:4:2}"
            sed -i "s|irc_nick3 = Elive_user|irc_nick3 = Elive_user_${LANG%%_*}-${NUMBERRANDOM}|" "${HOME}/.xchat2/xchat.conf"
        else
